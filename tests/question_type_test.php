...
class question_type_test extends \question_testcase {
    ...
    /** @var object sample data for reuse */
    protected $qdata;
     ...
     */
    public function test_export_to_xml_crossword_pre_upgrade_feedback(): void {
        $this->resetAfterTest();

        $exporter = new \qformat_xml();

        $xml = $exporter->writequestion($this->qdata);
        $expectedxml =
            '<!-- question: 8862  -->
...
';
     ...
     * @covers \qtype_crossword::export_to_xml
     */
    public function test_export_to_xml_crossword_after_upgrade_feedback(): void {
        $this->resetAfterTest();
        $qdata = $this->qdata;
        $qdata->options->words[0]->feedback = '<b>Feedback data</b>';
        $qdata->options->words[0]->feedbackformat = FORMAT_PLAIN;
        $exporter = new \qformat_xml();
