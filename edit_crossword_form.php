...
class qtype_crossword_edit_form extends question_edit_form {
    ...
    public function definition_after_data() {
        ...
        if (isset($submittedformat)) {
            $mform = $this->_form;
            foreach ($_POST['clue'] as $index => $c) {
                $elementname = "clue[$index]";
                $editor = $mform->getElement($elementname);
                $editor->setValue([
    ...
    protected function get_per_answer_fields($mform, $label, $gradeoptions,
        ...
        $coordinatesoptions = array_merge($coordinatesoptions, $coodinatesform);
    ...
    protected function add_per_answer_fields(&$mform, $label, $gradeoptions,
        ...
        $switchformatgroup[] = $mform->createElement('submit', 'saveupdate', get_string('switchformat', 'qtype_crossword'));
        $mform->registerNoSubmitButton('saveupdate');
    ...
    private function data_preprocessing_words(object $question): object {
        ...
        if (!empty($question->options->words)) {
            ...
            foreach ($question->options->words as $index => $answerdata) {
                ...
                $draftitemid = file_get_submitted_draft_itemid('clue['.$key.']');
                $clue[$key]['text'] = file_prepare_draft_area(
                    $draftitemid,
                    $this->context->id,
                    'question',
                    'clue',
                    !empty($answerdata->id) ? (int) $answerdata->id : null,
                    $this->fileoptions,
                    $answerdata->clue
                );
                $clue[$key]['itemid'] = $draftitemid;
                $clue[$key]['format'] = isset($answerdata->clueformat) ? $answerdata->clueformat : $defaultformat;
                ...
                $feedback[$key]['itemid'] = $draftitemid;
                $feedback[$key]['format'] = isset($answerdata->feedbackformat) ? $answerdata->feedbackformat : $defaultformat;
